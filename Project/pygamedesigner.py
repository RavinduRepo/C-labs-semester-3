...
import pickle
from ctypes import*
lib = CDLL('C:/Users/ravin/Documents/My Projects/c language/Pera c works/CO222/Project/Milestone.so')
...
white_v = -1 * np.ones(length**2 + length).reshape(length, length + 1)#[cell][wall]
white_h = -1 * np.ones(length**2 + length).reshape(length + 1, length)#[wall][cell] 
